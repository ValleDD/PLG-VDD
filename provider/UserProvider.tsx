...
import { UserContext, UserContextInterface, User} from "../context/UserContext";
...
export const UserProvider:React.FC<UserProviderProps>=({children}) =>{
    ...
    const login = async (name: string, password: string) => {
      try {
        const response = await fetch('http://192.168.1.34:8888/users/login', {
          method: 'POST',
          headers: {
        ...
        if (response.ok) {
         ...
         await AsyncStorage.setItem('authToken', userData.token || '');
          ...
          setUser({
            name: userData.name,
            password: userData.password, // Esto podría ser innecesario o un riesgo de seguridad, depende de tu aplicación
            isLoggedIn: true,
          });
    ...
    const logout = async() => {
      try {
        const response = await fetch('http://192.168.1.34:8888/users/logout', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // Include any necessary authentication headers or tokens
        },
        // Include any necessary payload for logout
      });

      if (response.ok) {
        // Clear local storage or perform additional cleanup
